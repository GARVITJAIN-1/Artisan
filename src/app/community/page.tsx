...
export default function Home() {
      ...
      if (searchTerm === "") {
        ...
        const results = allArtworks.filter(
          (artwork) =>
            ...
            (t.artworks[artwork.id]?.title.toLowerCase().includes(lowerCaseSearchTerm)) ||
            (t.artworks[artwork.id]?.artist.toLowerCase().includes(lowerCaseSearchTerm)) ||
  ...
  }, [searchTerm, t.artworks, artworks]);

  // Set initial artworks
  useEffect(() => {
    ...
    <div className="container py-8">
        ...
        <div className="relative w-full max-w-xl">
          ...
          />
           <Button 
            variant="ghost" 
            size="icon" 
            className={cn("absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full", isRecording && "text-red-500")}
      ...
      {filteredArtworks.length > 0 ? (
        ...
        <div className="text-center py-20">
          <h2 className="text-2xl font-headline">{t.home.noArtworksFound}</h2>
          <p className="text-muted-foreground mt-2">{t.home.adjustSearchTerms}</p>
        </div>
      )}
    </div>
  );
